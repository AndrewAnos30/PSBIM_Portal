    ...
    if (!empty($username)) {
        try {
            ...
            if ($member) {
                ...
                $checkAttendance = $pdo->prepare("SELECT attendance FROM am_attendance WHERE username = ?");
                ...
                if ($attendanceRecord && $attendanceRecord['attendance']) {
                    echo "<script>alert('Member already scanned for AM.');</script>";
                } else {
                    // Insert or update attendance
                    $sql = "
                        INSERT INTO am_attendance (username, attendance)
                    ...
                    echo "<script>alert('AM Attendance recorded successfully.');</script>";
                }
            } else {
...
<div id="cameraModal" class="camera-modal" aria-hidden="true" role="dialog" aria-label="QR scanner">
    ...
    <div class="camera-modal__panel" role="document">
        <header class="camera-modal__header">
            <h2 class="camera-modal__title">Scan QR Code (AM)</h2>
...
<div class="member-modal" id="memberModal" aria-hidden="true">
  ...
  <div class="member-modal__panel">
    <div class="member-modal__header">
      <h3 class="member-modal__title">Confirm Member Details</h3>
            ...
            <?php if (!empty($members)): ?>
                <?php foreach ($members as $record): ?>
                    ...
                    <tr data-id="<?= htmlspecialchars($record['id']) ?>"
                        ...
                        <td>
                            <div class="status-icon" style="background-color: <?= $record['am_attendance'] ? 'green' : 'transparent' ?>;">
...
document.getElementById("confirmAttendanceBtn").addEventListener("click", () => {
  const confirmAction = confirm("Confirm attendance for this member?");
  if (confirmAction) {
...
<style>
/* Member Confirmation Modal (styled like your dashboard) */
.member-modal {
  position: fixed;
  inset: 0;
  display: none;
...
.btn-confirm:hover { background-color: #800; }
</style>
